...
// import MintPageBg from '../assets/mint-page.png'
...
const MintPage = () => {
    ...
    const onPageLoad = async () => {
        ...
        try {
            ...
            setPresaleMaxNftPerWallet(weiToNumber(await contract.presaleMaxNFTPerWallet()))
            setMainSaleMaxNftPerWallet(weiToNumber(await contract.publicSaleMaxNFTPerWallet()))
    ...
    return <main className="mint-page-main">
        <div className=" section flex-column-center mint-bg">
            <h2>Mint your buni's</h2>

            <div className="flex mint-controls-container">
                <div className="mint-num-btns" onClick={decreaseMintNum}>-</div>
                <h2 className="mint-num">{mintQuantity}</h2>
                <div className="mint-num-btns" onClick={increaseMintNum}>+</div>
            </div>

            {
                walletAddress ?
                    <div>
                        <small>Connected Wallet: <br /> {walletAddress}</small>
                        <br />
                        <br />
                    </div>
            ...
            {
                !walletConnected ?
                    <div className="btn" onClick={connectWallet}>Connect wallet</div>
                    : <div className="flex">
                        <div className="btn connect-wallet-btn exit-min-btn" onClick={exitMint}>Exit mint</div>
                        <div className="btn mint-btn" onClick={mint}>Mint now</div>
                    </div>
            }
        </div>
